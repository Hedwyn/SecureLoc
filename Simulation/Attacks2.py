...
class AttackSimulation:
    ...
    def modify_distance(self,distance, ancher_id, distance_shift):
        """applies the attack distribution to the distance"""
        ## # TODO: AWGN
        if(ancher_id == '02' or ancher_id == '03'):
           distance = distance + distance_shift
        elif (ancher_id == '01' or ancher_id =='04'):
            distance = distance - distance_shift

        return(distance + self.distribution)

    def applies_attack_on_1sample(self,distance, ancher_id, distance_shift):
        ...
        if (SUCCESS):
            # modify distance
            modified_distance = self.modify_distance(distance, ancher_id, distance_shift)
...
class LogsAttacks(AttackSimulation):
    ...
    def __init__(self,success_rate = 0,dos_rate = 0,distribution = 0,name = "Sneaky Attacker", distance_shift = 2):
         self.success_rate = success_rate
         self.dos_rate = dos_rate
         self.distribution = distribution
         self.distance_shift = distance_shift

    def simulate_attack(self,logname_in,logname_out, distance_shift):
        """simulates the attack on the entire log"""

        with open(logname_in,'r') as log_in:
            try:
                log_out = open(logname_out,'a')
            except:
            ...
            for line in log_in:
                ...
                if "metadata" in json_sample:
                    ...
                    if distance is None:
                        continue
                    modified_distance = self.applies_attack_on_1sample(distance, json_sample["anchorID"], distance_shift)
...
for i in range(2, 22, 2):
    #print(i)
    distance_shift.simulate_attack(LOGIN,LOGOUT,i)
