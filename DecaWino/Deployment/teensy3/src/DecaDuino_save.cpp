...
void DecaDuino::readSpiSubAddress(uint8_t address, uint16_t subAddress, uint8_t* buf, uint16_t len) {
	...
	if ( subAddress < 0x80 ) {
		...
		sub_addr_lo = 1 | (subAddress & 0x7F);  // Mask register address (6bits)
		sub_addr_ho = subAddress>>8;
...
void DecaDuino::getAccumulatedCIR(int idx_start, int idx_end, uint16_t *re, uint16_t *im) {
	uint8_t buf[DWM1000_ACCUMULATOR_LENGTH];
	int i, length = idx_end - idx_start;
	uint32_t ui32t;
	...
	readSpiSubAddress(DWM1000_REGISTER_ACC_MEM, idx_start<<2, buf, 1 + length<<2);
	//readSpi(DWM1000_REGISTER_ACC_MEM, buf, length);

	for (i = 0 ; i < length; i++) {
		/* first two bytes are the real part */
		re[i] = decodeUint16(buf +  4 * i);
		im[i] = decodeUint16(buf + 2 + 4 * i);
		// Serial.println((int) re[i]);
		// Serial.println((int) im[i]);
		// Serial.print((int) buf[4 * i]);
		// Serial.print(";");
		// Serial.print((int) buf[4 * i + 1]);
		// Serial.print(";");
		// Serial.print((int) buf[4 * i + 2]);
		// Serial.print(";");
		// Serial.println((int) buf[4 * i + 3]);
	}
}
...
double DecaDuino::getFpPower(void) {
	...
	if (prf == 16) {
		// prf set to 16 MHz
		Serial.println("16 Mhz detected  for fp !");
...
double DecaDuino::getRSSI(void) {
	...
	if (prf == 16) {
		// prf set to 16 MHz
		Serial.println("16 Mhz detected  for rss !");
		A = DWM1000_PRF_16MHZ_CIRE_CONSTANT;
	}
	else {
		// prf set to 64 MHz
