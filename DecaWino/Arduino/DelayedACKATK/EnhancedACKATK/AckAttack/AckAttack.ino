...
uint64_t t2, t3, t_pre,t_to;      //temps de réception et temps de ré-émission
 ...
 */
    ...
    case TWR_ENGINE_STATE_WAIT_ACK:
      ...
      if (!decaduino.waitPreamble(500000)) {
        state = TWR_ENGINE_STATE_RX_ON;
        Serial.println("preamble wait timeout");
        
        
      }
      else {
       
        t_pre = decaduino.getPreambleTimestamp();

        

        state = TWR_ENGINE_STATE_SEND_ACK;
        Serial.println("new preamble received");
        evc_sfdto = decaduino.readSfdTimeoutCounter() ;
        
        while (decaduino.readSfdTimeoutCounter() - evc_sfdto < 1);
        t_to = decaduino.getSystemTimeCounter();
        decaduino.plmeRxDisableRequest();
        Serial.println((float) 15.65E-6*(t_to - t_pre));
        Serial.print("Symbols accumulated:");
        Serial.println(decaduino.getRxPacc());
        //Serial.print((int) (decaduino.getSfdtoTimestamp() - decaduino.getPreambleTimestamp()));
        // For attacks only
        ...
        if (!decaduino.pdDataRequest(txData, 0)) {
          Serial.println("PRE 64 failed");
        }
        while (!decaduino.hasTxSucceeded());
        Serial.println("Time for pre 64:");
        Serial.println((millis() - evc_sfdto));
        Serial.println("PRE 64 finished");
 ...
 */
    case TWR_ENGINE_STATE_MEMORISE_T3:
      Serial.println("PRE 1024 succeeded");
