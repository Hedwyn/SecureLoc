...
from ips.core.world import World
from ips.simulation.simulation import Simulator
from ips.nodes.anchor import Anchor
from ips.nodes.tag import Tag
from ips.localization.filter import Filter
if MEASURING:
    from ips.gui.sound import Melody
from ips.logs.json_logs import jsonLogs
from ips.logs.json_structures import dataset,position,metadata
from ips.gui.rendering_interface import Renderer
if (HEADLESS):
    from ips.gui.rendering_interface import taskMgr # TODO
...
class Application(Renderer):
        ...
        if (PLAYBACK):
            try:
                ...
                if list_of_files:
                    ...
                    print((PLAYBACK_DIR + '/*.json'))
                    print("No playback file available. Quitting")
                    return
                ...
                playback_dir = self.playback_path

                if not ( os.path.exists(playback_dir) ):
                    try:
                        os.mkdir(playback_dir)
                    except OSError:
                        print("Playback dir does not exist: " + str(playback_dir))
    ...
    def add_tag(self, tag_pos):
        ...
        new_tag = Tag(new_tag_name, 'magenta')
    ...
    def create_tags(self):
        ...
        for i in range(NB_BOTS):           
            self.tags[ bots_id[i] ]  = Tag( bots_id[i], TAG_COLOR)
